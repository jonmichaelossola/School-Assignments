...
public class CreatureHandler
{
	...
	*/
	public void updateCreatures() {
		...
		List<Creature> wolvesBirthed = new ArrayList<Creature>();
		for(Creature c : _creatures) {
				...
				if(c instanceof Aggressor && !(c instanceof Spawner)) {

					Creature target;
					if (c instanceof LaserMan) {
						if (_rand.nextInt(5) == _rand.nextInt(5)) {
							int x = 0;
							boolean found = false;
							while (getTarget(c, x, 0) == null &&  x < 200)
								x++;

							target = getTarget(c, x, 0);
						}
						else
							target = getTarget(c, 0, 0);
					}
					else
						target = getTarget(c, 0, 0);
				...
				if (c instanceof Spawner) {
					...
					// spawn a new wolf
					Creature wolf = (Creature)a;
					Wolf theWolf = (Wolf)wolf;
					if (target != null && !(target.isAlive()))
						wolvesBirthed.add(theWolf.spawnNewCreature());
				}
				
			}

			for (Creature birth : wolvesBirthed) {
				_creatures.add(birth);
			}
	}
}